...
public sealed class VisionClient
{
  ...
  /// </param>
  public VisionClient(ChatCompletionClient chatCompletionClient, HttpClient httpClient) =>
  ...
  private static void ValidateVisionModel(JsonObject request)
  {
    ...
    if (string.IsNullOrEmpty(model) || !VisionSettings.VisionModels.Contains(model))
    {
      throw new ArgumentException($"Invalid vision model. Must be one of: {VisionSettings.VisionModels}");
    }
  }
